...
def main():
    ...
    servers2 = {'us-east-1': ['ami-0a8b4cd432b1c3063', 'sg-024f8a033f5d4499e', (38.8051, -77.0470)],
                'us-east-2': ['ami-0231217be14a6f3ba', 'sg-039cdf5305a9721de', (40.4173, -82.9071)],
                'us-west-1': ['ami-01163e76c844a2129', 'sg-034e8622c6d9e9a87', (38.8374, -120.8958)],
                'us-west-2': ['ami-06cffe063efe892ad', 'sg-0e052facfd34bcdfa', (43.8041, -120.5542)],
                'ap-south-1': ['ami-03fa4afc89e4a8a09', 'sg-01515a9f3319bf590', (33.9249, 18.4241)],
...
def create_instance(region: str, image_id: str, sg: str, public: str, secret: str):
    ...
    :param region: where to create instance
    :param image_id: machine image
    ...
    while True:
        try:
            ...
            break
        except botocore.exceptions.ClientError or botocore.exceptions.EndpointConnectionError:
            pass
...
def terminate_instance(region: str, instance_id: str, public: str, secret: str):
    ...
    :param region: where to create instance
    :param instance_id: machine image
    :param public: public key
...
def run_command(ssh, cmd: str, timeout: int):
    ...
    :param timeout: how long to wait until timeout
    ...
    try:
        ...
        print(output)
        return output
    except socket.timeout or paramiko.ssh_exception.SSHException or paramiko.ssh_exception.SSHException:
        print('took a while')
        pass
    return stdout
...
def calculate_bandwidth(region1: str, image_id1: str, region2: str, image_id2: str, sg1: str, sg2: str,
    ...
    :param region1: where to create instance 1
    :param image_id1: machine image for instance 1
    :param region2: where to create instance 2
    :param image_id2: machine image for instance 2
    :param sg1: security group for instance 1
    :param sg2: security group for instance 2
    :param public: public key
    :param secret: private key
    :return:
    """
    ...
    while True:
        try:
            ...
            if region1 == 'us-east-1':
                raw_output = run_command(ssh,
                                         f"iperf3 -c ec2-{receiver_ip.replace('.', '-')}.compute-1."
                                         f"amazonaws.com -t 10 -f 'm'", 15)
            else:
                raw_output = run_command(ssh,
                                         f"iperf3 -c ec2-{receiver_ip.replace('.', '-')}.{region1}."
                                         f"compute.amazonaws.com -t 10 -f 'm'", 15)
            ssh.close()
